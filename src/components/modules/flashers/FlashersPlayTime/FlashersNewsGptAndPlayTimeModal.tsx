...
import FlashersNewsGptAndPlayTimeContent from "./FlashersNewsGptAndPlayTimeContent";
...
const FlashersNewsGptAndPlayTimeModal = ({
  setIsOpenModal,
}: {
  setIsOpenModal: React.Dispatch<React.SetStateAction<boolean>>;
}) => {
  const [, setSearchParams] = useSearchParams();
  const { selectedFlashersHeadlines } = useFlasherContext();
  ...
  const handleSubmitPromptData = (prompt: string) => {
    const data = selectedFlashersHeadlines.map((item) => {
      return {
        text: item.ocrResult,
    ...
    <Portal>
      <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/70">
        <div className="grid h-[90vh] w-11/12 grid-rows-[auto_1fr] gap-2 rounded-md bg-white p-1">
          <header className="grid grid-cols-1 grid-rows-2 gap-2 bg-gray-100 py-2.5 lg:grid-cols-[3fr_2fr] lg:grid-rows-1">
            ...
            <div className="flex items-center justify-end gap-1">
              ...
              <NewGptPromptsButtons
                ...
                selectedDataLength={selectedFlashersHeadlines.length}
          ...
          <FlashersNewsGptAndPlayTimeContent />
        </div>
      </div>
...
export default FlashersNewsGptAndPlayTimeModal;
