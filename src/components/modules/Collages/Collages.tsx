...
import useGetAllCollages from "../../../api/useGetAllCollages";
import { useEffect } from "react";
import CircularLoader from "../../uiComponents/CircularLoader";
import MySingleCollageCard from "../Tickers/MySingleCollageCard";
import toast from "react-hot-toast";
import placeholderImage from "../../../assets/placeholder-loading-collages.png";

function Collages({ myTeamCollages }: { myTeamCollages: boolean }) {
  const {
    data,
    isLoading,
    isError,
    ...
    fetchNextPage,
    isFetchingNextPage,
  } = useGetAllCollages({ myTeamCollages });
  ...
  return (
    <div className="overflow-y-auto">
      {allCollages?.map((singleCollage, index) => {
        return (
          <MySingleCollageCard
            singleCollage={singleCollage}
            key={index}
            myTeamCollages={myTeamCollages}
          />
        );
      })}
