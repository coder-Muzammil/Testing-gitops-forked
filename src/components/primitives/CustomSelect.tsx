...
const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  const handleOptionClick = (option: Option) => {
    setSearchParams((currentParams) => {
      currentParams.set("hours", String(option.value));
    ...
    <div className="relative w-full " ref={ref as LegacyRef<HTMLDivElement>}>
      ...
      >
        <div className="flex items-center">
          <PlaceholderIcon className="mr-2 text-gray-500 dark:text-white" />
          {selectedOption ? (
            <span>
            ...
            <span className="text-gray-500 dark:text-white">{placeholder}</span>
          )}
        </div>
        <FaChevronDown
          className={twMerge(
